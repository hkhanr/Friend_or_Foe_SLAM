...
//vector < vector<cv::KeyPoint> > DB_keypoints;	


    Mat descriptors1;
    orb->detectAndCompute(image, mask1, keypoints1, descriptors1);
	//DB_keypoints.push_back(keypoints1);

       vector<cv::Mat> Desc = Converter::toDescriptorVector(descriptors1);
      mpORBVocabulary->transform(Desc,D_Bow,D_FeatVec,4);

	DB_D_Bow.push_back(D_Bow);
}
    ...
    while(ros::ok())
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(DetectLoop())
            {
               ...
               if(ComputeSim3())
               { 
			cout << "Loop Closure detected. Checking if its true or false."<<endl;
		if (CheckDecieved())
		  {
...
/*	vector <MapPoint*> po =vvpMapPointMatches[0];
        MapPoint* P=po[0];
         int ind =P->GetIndexInKeyFrame(pKF);
   cv::KeyPoint &Kpp = pKF->GetKeyPointUn(ind);*/
        if(nmatches<20)
        {
            vbDiscarded[i] = true;
            continue;
...
bool LoopClosing::CheckDecieved()
{
cout<<endl;
cout<<" KeyFrame "<<mpCurrentKF->mnId<<endl;
     DBoW2::BowVector KF_Bow = mpCurrentKF->GetBowVector();

for(unsigned int i = 0; i < DB_D_Bow.size(); i++)
{
d_Bow=DB_D_Bow[i];

     float D_score = mpORBVocabulary->score(KF_Bow, d_Bow);
     cout<<"	Score with image "<<i<<" = "<<D_score<<endl;
	if (D_score > 0.008)
	{
		mpKeyFrameDB->add(mpCurrentKF);
		mpCurrentKF->SetErase();
		cout << " False Loop Closure"<<endl;
		return false;   
	}

}
		cout << "True Loop Closure"<<endl;
		return true;   
}
